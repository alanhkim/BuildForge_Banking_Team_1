...
@description('Policy/initiative parameter values, keyed by parameter name, e.g. { logAnalytics: { value: \'<workspaceResourceId>\' } }.')
...
@description('Role definition IDs (GUIDs, not full resource IDs) to grant to the assignment\'s system-assigned identity at this subscription scope. Required for DeployIfNotExists/Modify initiatives to actually remediate.')
...
resource roleAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for roleId in roleDefinitionIds: if (identityType != 'None') {
  name: guid(subscription().id, assignmentName, roleId)
  properties: {
