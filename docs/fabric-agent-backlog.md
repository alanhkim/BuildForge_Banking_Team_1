...
- Live `FabricTest` metadata confirms an active prompt agent at version `4` using `gpt-5`, the `responses` protocol, Entra authorization, and the `fabric_dataagent_preview` tool connection `fabric_dataagent_preview_941627`.
- The Fabric Data Agent smoke prompt set is versioned in `data/fabric_data_agent_smoke_prompts.yaml`; validation details are in `docs/fabric-data-agent-validation.md`.
- Fabric workspace roles now include the Foundry agent instance identity and blueprint identity as `Contributor`; after publishing the Fabric Data Agent, `FabricTest` version `4` successfully invokes the Fabric Data Agent tool through Azure AI Projects SDK `agent_reference`.
...
- Purpose-built Foundry prompt agents now exist for Control Mapper, Gap Analyst, Remediation Planner, Compliance Score Narrator, Executive Q&A, and Audit/Lineage. Their active names, versions, identities, and validation state are recorded in `docs/foundry-fabric-agents.md`.
- Direct Fabric Data Agent REST invocation still returns `404`; app integration should use Foundry `agent_reference` invocation.
