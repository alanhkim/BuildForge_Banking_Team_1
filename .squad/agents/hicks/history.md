...
### 2026-07-21 (late evening, hardening #3) — Team update: 2 new IDF-parenthesisation regression tests + 1 resurrected orphan-block test in `test_lakehouse.py` — total 55 → 58; 5 pre-existing IDF assertions updated to expect parenthesised form (no assertions weakened)
- Bishop shipped a fourth same-session hardening on top of `891eb2b` (the 21:30:00Z type-coercion drop). Triggering failure was `Cannot infer common argument type for logical boolean operation Utf8 OR Boolean` on `business_processes` MERGE (note **`Utf8`**, not `LargeUtf8` — the type-cast layer from `891eb2b` is confirmed working; this was a separate parser bug underneath). Root cause: DataFusion's SQL parser (delta-rs 1.6.2) mis-groups `IS DISTINCT FROM` inside `OR` chains when the predicate has 2+ clauses. Fix: wrap each IDF clause in explicit parens. Applied uniformly across all cardinalities. Governing decision at the top of `.squad/decisions.md`. Test count **55 → 58**.
- **Test surface update:** `tests/test_lakehouse.py` is now **58/58 green** in **2.03s**. Trajectory: 34 → 41 → 46 → 55 → **58**. The +3 vs the +2 expected count is one resurrected orphan-block test (see next bullet).
- **⚠️ Orphan test block from `891eb2b` is RESURRECTED, not just flagged.** The orphan at L1655-1671 (from `tests/test_lakehouse.py`, flagged in the hardening #2 entry below and running silently inside `test_write_delta_table_first_write_path_unchanged`) is now a first-class test: `test_write_delta_table_raises_when_table_name_not_in_primary_keys_map`. Only the `def test_...` line had been stripped by an earlier edit — the docstring `"""A Parquet file whose stem is not in _TABLE_PRIMARY_KEYS must raise LakehouseWriteError..."""` and the body were intact. Bishop restored the `def` line; the resurrected test now runs and passes on its own. **Cross off the "TODO for future cleanup" note from hardening #2 — it's done.**
- **The 2 genuinely new tests:**
  1. `test_write_delta_table_update_predicate_parenthesizes_each_clause` — the exact `business_processes` failure shape (3 non-PK PascalCase compare columns). Asserts each clause parenthesised, `) OR (` joiner present, exactly 3 `(target."` openings, full expected string in order. **This is the guard that would have caught the bug on the first MERGE-path run against `business_processes`.**
  2. `test_write_delta_table_update_predicate_parenthesizes_single_clause` — consistency guard for the 1-clause case (`dim_control`, 1 non-PK non-`as_of` column). Asserts `(target."name" IS DISTINCT FROM source."name")` exactly, and that `" OR "` is absent. Locks the "parenthesise even the single-column case for consistency" invariant so a future schema change that promotes a table to 2+ compare columns doesn't silently drop back into the broken form.
- **5 pre-existing IDF assertions updated to expect parenthesised form. NO assertions weakened.** Shape/intent unchanged — only the exact expected string changed. Affected tests: `test_write_delta_table_update_predicate_excludes_as_of_and_pk`, `test_write_delta_table_composite_pk_relationships`, `test_write_delta_table_merges_when_target_has_pascalcase_columns`, `test_write_delta_table_merges_when_target_has_mixed_case_columns` (plus one internal assertion in a related test). Pattern to preserve: when the emitted SQL shape changes but the semantic is identical, update the expected-string in place — do NOT relax to a substring match or a shape-only assertion, because a future regression could then land un-parenthesised again without failing the test.
- **Zero test-shim changes required this drop.** `_build_merge_recording_dt` was **not modified** — the predicate string is captured via the same seam that already exists. Pattern payoff continues: extending the fake once for case-alignment now supports three hardenings across the same seam. Preserve that pattern.
- **Pattern to keep: "add a test per invariant, not per code path."** The two new tests encode two design invariants (each clause is atomic; single-clause tables are parenthesised too for future-proofing). Same discipline as `891eb2b`, `775c857`, and the MERGE landing.
- **Baseline unchanged:** the 23-failure pre-existing baseline still stands (env-drift + asyncio config + one interpreter contract drift — none touch OneLake). Boundary-error-class discipline preserved.
